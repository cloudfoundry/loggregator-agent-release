...
var _ = Describe("EgressFactory", func() {
	...
	BeforeEach(func() {
		...
		f = syslog.NewWriterFactory(&tls.Config{}, &tls.Config{}, syslog.NetworkTimeoutConfig{}, sm, syslog.LoggregatorEmitter{}) //nolint:gosec
	})
	...
	Context("when the url begins with https", func() {
		It("returns an single https writer", func() {
			...
			writer, err := f.NewWriter(urlBinding, syslog.LoggregatorEmitter{})
			Expect(err).ToNot(HaveOccurred())
	...
	Context("when the url begins with syslog-tls://", func() {
		...
		Context("when the private key is not passed", func() {
			It("the certificate is ignored", func() {
				...
				_, err = f.NewWriter(urlBinding, syslog.LoggregatorEmitter{})
				Expect(err).ToNot(HaveOccurred())
			})
		})
