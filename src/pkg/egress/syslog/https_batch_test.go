...
var _ = Describe("HTTPS_batch", func() {
	...
	It("testing simple appending of one log", func() {
		...
		Eventually(drain.getMessagesSize, 180*time.Millisecond).Should(Equal(2))
	...
	It("test batch dispatching with all logs in a given timeframe", func() {
		...
		Eventually(drain.getMessagesSize, 180*time.Millisecond).Should(Equal(5))
	})
	...
	It("triggers multiple flushes when exceeding the batch", func() {
		...
		Eventually(drain.getMessagesSize, 180*time.Millisecond).Should(Equal(15))
