...
var _ = Describe("PromScraper", func() {
	...
	It("returns a error if port is not set", func() {
		...
		Expect(string(buffer.String())).To(MatchRegexp("Prom scraper config at /tmp/[0-9]*/prom_scraper_config.yml[0-9]* does not have a valid port - skipping this config file"))
	})
