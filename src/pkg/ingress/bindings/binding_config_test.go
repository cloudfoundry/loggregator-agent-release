...
var _ = Describe("Drain Param Config", func() {
	...
	It("sets drain filter appropriately", func() {
		testCases := []struct {
			...
			{
				name:     "include-log-types= defaults to no filtering",
				url:      "https://test.org/drain?include-log-types=",
				expected: nil,
			},
			{
				name:     "exclude-log-types= defaults to no filtering",
				url:      "https://test.org/drain?exclude-log-types=",
				expected: nil,
			},
			{
